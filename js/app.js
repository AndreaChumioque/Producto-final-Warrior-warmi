...
function begin() {
  ...
  function infoSprint(event) {
    ...
    var hseTarget = 0;
    for (var i = 0; i < students.length; i++) {
      if (students[i]['active'] === true && students[i].sprints[selectSprint.value - 1].score.tech > 1260) {
        techTarget++;
      }
      if (students[i]['active'] === true && students[i].sprints[selectSprint.value - 1].score.hse > 840) {
        hseTarget++;
      }
    }
